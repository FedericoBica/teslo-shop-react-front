...
export const CustomHeader = () => {
  ...
  const handleSearch = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    const newSearcParams = new URLSearchParams();

    if(!query) {
      newSearcParams.delete('query')
    } else {
      newSearcParams.set('query', inputRef.current!.value);
    }

    setSearchParams(newSearcParams);
  }
      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            ...
            <Link to="/gender/kids" className={cn(`text-sm font-medium transition-colors hover:text-primary`,
